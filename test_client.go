...
// 运行所有测试的主函数
func runTests() {
	fmt.Println("🚀 开始测试 DeepSeek API 代理服务器")
	fmt.Println("==========================================")
	...
	// 创建测试客户端
	client := NewTestClient(baseURL, apiKey)
	...
	tests := []struct {
		...
		{"健康检查", client.TestHealth},
		{"模型列表", client.TestModels},
		{"聊天完成", client.TestChatCompletion},
		{"流式聊天", client.TestStreamingCompletion},
	}
	...
	if failed == 0 {
		fmt.Println("🎉 所有测试都通过了！代理服务器工作正常。")
	} else {
		fmt.Printf("⚠️  有 %d 个测试失败，请检查服务器配置。\n", failed)
	}
}